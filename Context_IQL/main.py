import os
os.environ['D4RL_SUPPRESS_IMPORT_ERROR'] = '1'
import gym
import d4rl
import numpy as np
from tqdm import tqdm
from absl import flags, app
import tensorflow as tf
import time

import agent
import memory

FLAGS = flags.FLAGS
flags.DEFINE_string('env', 'halfcheetah-medium-v2', 'Environment name')
flags.DEFINE_integer('seed', 0, 'Seed for random number generators')
flags.DEFINE_float('max_timesteps', 1e6, "Max time steps")
flags.DEFINE_integer("eval_episodes", 10, "Evaluation Episode")
flags.DEFINE_integer('batch_size', 256, "batch size for both actor and critic")
flags.DEFINE_integer('context_dim', 16, "context dimension")
flags.DEFINE_integer('log_freq', int(1e3), 'logging frequency')
flags.DEFINE_float('eval_freq', 1e3, "evaluation frequency")
flags.DEFINE_float("temperature", 3.0, "temperature")
flags.DEFINE_float("expectile", 0.8, "expectile")
flags.DEFINE_float("tau", 0.005, "tau")
flags.DEFINE_float("discount", 0.99, "Discount factor")

def get_dummy_dataset(state_dim, action_dim, num_samples=10000):
    return {
        'observations': np.random.rand(num_samples, state_dim),
        'actions': np.random.rand(num_samples, action_dim),
        'next_observations': np.random.rand(num_samples, state_dim),
        'rewards': np.random.rand(num_samples),
        'terminals': np.random.randint(0, 2, size=num_samples, dtype=bool),
    }

def eval_policy(policy, env_name, seed, mean, std, seed_offset=100, eval_episodes=10):
    eval_env = gym.make(env_name)
    eval_env.seed(seed + seed_offset)

    avg_reward = 0.
    for _ in range(eval_episodes):
        state, done = eval_env.reset(), False
        # Create a dummy context for evaluation
        context = np.zeros((1, FLAGS.context_dim), dtype=np.float32)
        while not done:
            state = (np.array(state).reshape(1,-1) - mean)/std
            action = policy.select_action(state, context, seed)
            state, reward, done, _ = eval_env.step(action)
            avg_reward += reward

    avg_reward /= eval_episodes

    print("---------------------------------------")
    # The d4rl score is not available without d4rl, so we'll just print the average reward.
    print(f"Evaluation over {eval_episodes} episodes: {avg_reward:.3f}")
    print("---------------------------------------")
    return avg_reward


def main(argv):

    policy_name = 'Context_IQL'
    file_name = f"{policy_name}_{FLAGS.env}_{FLAGS.seed}"
    print("---------------------------------------")
    print(f"Policy: {policy_name}, Env: {FLAGS.env}, Seed: {FLAGS.seed}")
    print("---------------------------------------")

    if not os.path.exists('./results'):
        os.makedirs('./results')

    if not os.path.exists('./models'):
        os.makedirs('./models')

    env = gym.make(FLAGS.env)

    # Set seed
    env.seed(FLAGS.seed)
    env.action_space.seed(FLAGS.seed)
    env.observation_space.seed(FLAGS.seed)
    tf.random.set_seed(FLAGS.seed)
    np.random.seed(FLAGS.seed)

    state_dim = env.observation_space.shape[0]
    action_dim = env.action_space.shape[0]

    kwargs = {
        'state_dim': state_dim,
        'action_dim': action_dim,
        'context_dim': FLAGS.context_dim,
        'discount': FLAGS.discount,
        'tau': FLAGS.tau,
        'temperature': FLAGS.temperature,
        'expectile': FLAGS.expectile,
        'seed': FLAGS.seed
    }

    policy = agent.IQL(**kwargs)

    context_data_dim = 32
    replay_buffer = memory.ReplayBuffer(state_dim, action_dim, context_data_dim)
    dummy_dataset = get_dummy_dataset(state_dim, action_dim)
    replay_buffer.convert_D4RL(dummy_dataset)
    mean, std = replay_buffer.normalize_states()

    evaluations = []
    for t in tqdm(range(int(FLAGS.max_timesteps))):
        policy.train(replay_buffer, FLAGS.batch_size)

        if (t + 1) % FLAGS.eval_freq == 0:
            evaluations.append(eval_policy(policy, FLAGS.env, FLAGS.seed, mean, std, eval_episodes=FLAGS.eval_episodes))
            np.save(f"./results/{file_name}", evaluations)


if __name__ == '__main__':
    app.run(main)